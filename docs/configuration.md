...
<tr><td><code>TEST_SUITE_MAX_CONCURRENT_REVIEWS</code></td><td><code>test_suite_max_concurrent_reviews</code></td><td><code>8</code></td><td>Test Suite (RTM) per-job item concurrency: <code>_run_batch_review</code> fans items out with <code>asyncio.gather</code> under this <code>Semaphore</code> width, a soft cap above the RPM/TPM limiter. Also bounds the embedded RTM subgraph fan-out inside a hazard record <span class="src">qaai/core/config.py, qaai/api/services.py, qaai/agents/hazard_risk_reviewer/nodes.py</span></td></tr>
<tr><td><code>HAZARD_MAX_CONCURRENT_REVIEWS</code></td><td><code>hazard_max_concurrent_reviews</code></td><td><code>1</code></td><td>Hazard records per-job concurrency. Defaults to <code>1</code> (sequential) so the first record warms the shared <code>DD-*</code>/<code>REQ-*</code> cache before the next runs; each record still fans its own summarizers + requirement reviews out concurrently <span class="src">qaai/core/config.py, qaai/api/services.py</span></td></tr>
<tr><td><code>TEST_CASE_MAX_CONCURRENT_REVIEWS</code></td><td><code>test_case_max_concurrent_reviews</code></td><td><code>8</code></td><td>Single Test Case reviewer per-job item concurrency (same semantics as the Test Suite knob) <span class="src">qaai/core/config.py, qaai/api/services.py</span></td></tr>
...
Within one review job, items run **concurrently** under a per-reviewer semaphore: `_run_batch_review` fans them out with `asyncio.gather` under a `Semaphore` sized by that reviewer's knob — `TEST_SUITE_MAX_CONCURRENT_REVIEWS` (8), `TEST_CASE_MAX_CONCURRENT_REVIEWS` (8), or `HAZARD_MAX_CONCURRENT_REVIEWS` (**1**). The hazard reviewer defaults to **1** (records processed sequentially) so the first record warms the shared `DD-*`/`REQ-*` cache before the next starts — while each record still fans its own summarizers and requirement reviews out concurrently, with the embedded RTM subgraph fan-out bounded by `TEST_SUITE_MAX_CONCURRENT_REVIEWS`. An item that errors or produces an incomplete result has only its own run's cache entries purged (`purge_run`) so a failed attempt is never reused, without aborting the rest of the batch.
