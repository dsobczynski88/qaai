...
- **RBAC only gates *who may run*, not the options.** The SPA's role model (admin / reviewer / viewer) enables or disables the **Run** and **Feedback upload** actions; it never changes what an option means. It is UX gating only and is not yet enforced server-side, so a direct API caller is unaffected <span class="src">qaai/web/src/constants.ts:10-18</span>. See [Frontend &amp; RBAC → RBAC model](design/frontend_vue_rbac.html#rbac).
